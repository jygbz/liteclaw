...
from liteclaw.core.skill_loader import load_dynamic_skills, get_skill_count, clear_skill_cache
...
def benchmark():
    """性能基准测试"""
    print("=" * 70)
    print("LiteClaw 懒加载性能基准测试")
    print("=" * 70)
    
    # 创建临时测试目录
    temp_dir = tempfile.mkdtemp(prefix="liteclaw_bench_")
    
    # 修改环境变量
    original_env = os.environ.get("LITECLAW_WORKSPACE")
    os.environ["LITECLAW_WORKSPACE"] = temp_dir
    ...
    try:
        ...
        for num_skills in test_sizes:
            ...
            # 重新加载配置
            import importlib
            import liteclaw.core.config as config_module
            import liteclaw.core.skill_loader as skill_loader_module
        ...
        if original_env is not None:
            os.environ["LITECLAW_WORKSPACE"] = original_env
        else:
            os.environ.pop("LITECLAW_WORKSPACE", None)
